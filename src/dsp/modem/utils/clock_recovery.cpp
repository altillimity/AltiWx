...
static constexpr int NUM_STEPS = 128;

// Interpolator taps from GNU Radio.
// See ./support/generate_interpolator_taps.py for more info.
static const float mmseTaps[NUM_STEPS + 1][NUM_TAPS] = {
...
ClockRecovery::ClockRecovery(uint32_t sampleRate, float symbolRate)
{
  mu_ = 0.5f;
  omega_ = ((float)sampleRate / (float)symbolRate);
  setLoopBandwidth(0.005f);

  // Allow 0.2% deviation from expected omega.
  constexpr auto deviate = 0.005f;
...
void ClockRecovery::setLoopBandwidth(float bw)
{
  ...
  muGain_ = /*0.01;                  // 8.7e-3;//*/ (4 * damp * bw) / (1.0 + 2.0 * damp * bw + bw * bw);
  omegaGain_ = /*pow(0.01, 2) / 4.0; // pow(8.7e-3, 2) / 4.0f;//*/ (4 * bw * bw) / (1.0 + 2.0 * damp * bw + bw * bw);
}

std::vector<std::complex<float>> ClockRecovery::work(std::vector<std::complex<float>> in, size_t size)
{

  std::vector<std::complex<float>> toReturn;
  ...
  tmp_.insert(tmp_.end(), in.begin(), in.end());
